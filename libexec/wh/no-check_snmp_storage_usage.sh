...
Usage ()
{
	...
	echo
	echo "'exclude string' (optional)"
	echo "  string to exclude from volume list; can be a regex"
	echo
        echo "Options:"
        echo
        echo "-h, --help"
	...
	echo "  no-check_snmp_storage_usage public server12 90 95 '(this|that)'"
	echo
	echo
}
...
do
	...
	then
		...
		echo "${description[i]}" | egrep "$regex"  > /dev/null 2>&1
		errorcode="$?"
	fi
	if [ "$errorcode" -eq "1" ]
	then
