...
url = "http://#{optHash['host']}:#{optHash['port']}/jolokia/read/org.apache.activemq:type=Broker,brokerName=*,destinationType=Queue,destinationName=#{optHash['queue']}/#{optHash['attribute']}"
begin
  http = Curl.get(url)
...
if json_data.has_key? 'error_type'
  if optHash['queue'].start_with?('FAILED.') && json_data['error_type'] == 'javax.management.InstanceNotFoundException'
    puts "OK: no dead letter queue found.|#{optHash['attribute']}=0"
    exit 0
  else
