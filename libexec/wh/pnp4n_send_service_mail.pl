...
my $host               = hostname();
my @datacenter         = split (/\./, $host);
my $dc                 = $datacenter[-3];
# The version of this script
my $Version            ='1.7.3';
# the sender e-mail address to be seen by recipients
my $mail_sender        = uc($dc) . " Nagios <" . $dc . "nagios\@reachlocal.com>";
# The Nagios CGI URL for integrated service links
my $nagios_cgiurl      = "https://monitoring." . $dc . ".reachlocal.com/nagios/cgi-bin";
# Here we define a simple HTML stylesheet to be used in the HTML header.  Original font Verdana, sans-serif 10pt
my $html_style         = "body {text-align: right; font-family: helvetica, arial; font-size: 10pt;}\n"
                       . "img.logo {float: left; margin: 5px 5px 5px; vertical-align: middle}\n"
                       . "span {font-family: helvetica, arial; font-size: 10pt;}\n"
                       . "th {white-space: nowrap; }\n"
                       . "th.even {background-color: #c9c9c9; }\n"
                       . "td.even {background-color: #c9c9c9;}\n"
                       . "th.odd {background-color: #eaeaea; }\n"
                       . "td.odd {background-color: #eaeaea;}\n"
                       . "th,td {font-family: helvetica, arial ; font-size: 9pt; text-align:left; }\n"
                       . "th.customer {width: 400px; background-color: #004488; color: #ffffff;}";
my $table_size         = "375px";       #"600px";
my $header_size        = "100px";       #"180px";
my $data_size          = "350px";       #"420px";
my $debugtables        = "<br>\n";
...
# ########################################################################
my $test_host          = "padm005.rlcorp.local";    # existing host in PNP4Nagios
my $test_service       = "CPU Load"; # existing services in PNP4nagios
...
# ########################################################################
my $pnp4nagios_url     = "https://monitoring." . $dc . ".reachlocal.com/pnp4nagios";
my $graph_history      = 9; # in hours, a good range is between 12...48
...
# ########################################################################
my $o_smtphost         = "relayhost." . $dc . ".reachlocal.com";
my $domain 	       = "\@reachlocal.com"; # this is only for -g groups
my @listaddress        = ();
...
# ########################################################################
my $logofile = "/usr/local/nagios/share/images/rllogo.png";
...
sub create_test_data {
  if (! defined($o_customer)){         $o_customer         = "";}
  if (! defined($o_notificationtype)){ $o_notificationtype = "PROBLEM";}
  if (! defined($o_servicestate)){     $o_servicestate     = "WARNING";}
  if (! defined($o_hostname)){         $o_hostname         = $test_host;}
  if (! defined($o_hostalias)){        $o_hostalias        = "padm005.rlcorp.local";}
  if (! defined($o_hostaddress)){      $o_hostaddress      = "10.10.26.3";}
  if (! defined($o_hostgroup)){        $o_hostgroup        = "Windows Servers";}
  if (! defined($o_servicedesc)){      $o_servicedesc      = $test_service;}
  if (! defined($o_servicegroup)){     $o_servicegroup     = "Base Windows Checks";}
  if (! defined($o_datetime)){         $o_datetime         = `date`;}
  if (! defined($o_serviceoutput)){    $o_serviceoutput    = "This is only a test. Really Really Really Really Really Really Really long";}
  if (! defined($o_notificationauth)){ $o_notificationauth = "";}
  # Setting the keyword "email-debug" in the notification comment below triggers the creation of debug tables
  if (! defined($o_notificationcmt)){  $o_notificationcmt  = "Service notification test message";}
...
sub create_message_html {
 ...
 # }
  ...
  $html_msg .= "<th width=$header_size class=even>$language{$land}{'B'}:</th>\n"
            . "<td bgcolor=$colors{$o_notificationtype}>\n"
            . "$o_notificationtype</td></tr>\n"
            . "<tr><th class=odd>$language{$land}{'C'}:</th><td bgcolor=#eaeaea>$o_servicedesc</td></tr>\n";
...
#  else { $html_msg  .= $o_servicegroup; }
  ...
  $html_msg .= "</td></tr>\n"
            . "<tr><th class=odd>$language{$land}{'D'}:</th><td bgcolor=$colors{$o_servicestate}>$o_servicestate</td></tr>\n"
            . "<tr><th class=even>$language{$land}{'F'}:</th><td bgcolor=#eaeaea class=even>\n";
  ...
  $html_msg .= "</td></tr>\n"
            .  "<tr><th class=odd>$language{$land}{'G'}:</th><td bgcolor=#c9c9c9>\n";
...
#            . "<tr><th class=even>$language{$land}{'H'}:</th><td class=even>$o_hostalias</td></tr>\n"
            . "<tr><th class=odd>$language{$land}{'I'}:</th><td bgcolor=#eaeaea>$o_hostaddress</td></tr>\n"
            . "<tr><th class=even>$language{$land}{'J'}:</th><td bgcolor=#c9c9c9 class=even>\n";
  ...
  $html_msg .= "</td></tr>\n"
            .  "<tr><th class=odd>$language{$land}{'K'}:</th><td bgcolor=#eaeaea>$o_datetime</td></tr>\n";
  ...
  if (defined($graph_img) && $o_format eq "graph") {
    ...
    $html_msg .= "<img src=\"cid:$graph_id\" width=\"500\">\n";
  } 
...
sub set_subject {
  ...
  my %lang =  ('en' => "** $o_notificationtype Service Alert: $o_hostname/$o_servicedesc is $o_servicestate **",
