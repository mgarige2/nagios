...
my $Attribute = "StorePercentUsage";
my $ResponseValue = 0;
my $ReturnMsg = "";
my $ExitLevel = 0;
...
if ( $ExitLevel == 0 )
{
	...
	$ChkResponse = $response->value();
		...
		if ( $ChkResponse >= $Opts{c} )
		{
			...
			$ExitLevel = 2;
			$ReturnMsg="Critical: Store usage >= $Opts{c} %($ChkResponse).|storage_percentage=$ChkResponse%";
		}
		if ( $ChkResponse >= $Opts{w} && $ExitLevel == 0)
		{
			# we are over the warning threshold
			$ExitLevel = 1;
			$ReturnMsg="Warning: Store usage >= $Opts{w} %($ChkResponse).|storage_percentage=$ChkResponse%";
		}
...
if ( $ExitLevel == 0 )
{
    $ReturnMsg="OK : Store usage at $ChkResponse %|storage_percentage=$ChkResponse%";
}
print "$ReturnMsg\n";
exit $ExitLevel;
