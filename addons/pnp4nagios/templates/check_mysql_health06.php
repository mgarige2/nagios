...
foreach ($DS as $i) {
    ...
    if(preg_match('/^slave_lag$/', $NAME[$i])) {
        ...
        $opt[$defcnt] = "--vertical-label \"seconds\" --title \"Slave Delay for $hostname\" ";
        $def[$defcnt] = "";
	$def[$defcnt] .= "DEF:slavelag=$RRDFILE[$i]:$DS[$i]:MAX ";
	$def[$defcnt] .= "CDEF:slavelag$defcnt=slavelag,FLOOR " ;
	$def[$defcnt] .= "LINE1:slavelag$defcnt#f0f ";
	$def[$defcnt] .= "VDEF:vslavelag=slavelag,LAST " ;
 	$def[$defcnt] .= "GPRINT:vslavelag:\"is %.0lf Seconds \" ";
	$defcnt++;
    }
