...
# set initial values
	...
	if ($VAL['MAX'] != "") {
		...
		#$min = pnp::adjust_unit( $VAL['MIN'].$unit,1024,$fmt );
		$max = pnp::adjust_unit( $VAL['MAX'].$unit,1024,$fmt );
		$upper = "-u $max[1] ";
		$maximum = "of $max[1] $max[2]$pct used";
		#$minimum = "of $min[1] $min[2]$pct used";
		$label = $max[2];
		$divis = $max[3];
		$return = '';
	}
	...
	# set graph labels
	$opt[$KEY]     = "--vertical-label $label -l 0 $upper --title \"$hostname $ds_name[$KEY]\" ";
	# Graph Definitions
	$def[$KEY]     = rrd::def( "var1", $VAL['RRDFILE'], $VAL['DS'], "AVERAGE" ); 
	# "normalize" graph values
	$def[$KEY]    .= rrd::cdef( "v_n","var1,$divis,/");
	$def[$KEY]    .= rrd::area( "v_n", "#2E64FE",  $ds_name[$KEY] );
	...
	$def[$KEY]    .= rrd::gprint( "v_n", "LAST", "$fmt $label$pct $minimum");
	...
	if ($VAL['MAX'] != "") {
		...
		$def[$KEY] .= rrd::hrule( $max[1], "#003300", "TOTAL  $max[0] \\n");
	}
