...
if($display_traffic == 1){
    ...
    $def[$num_graph] .= rrd::cdef    ("bits_out_neg", "bits_out_redef,-1,*");
    $def[$num_graph] .= rrd::area    ("bits_in_redef",  '#32CD32', 'in_bps        ');
    $def[$num_graph] .= rrd::line1   ("bits_in_redef", '#000000') ;
    $def[$num_graph] .= rrd::gprint  ("bits_in_redef",  array("LAST","MAX","AVERAGE"), "%8.2lf%Sbps");
    $def[$num_graph] .= rrd::area    ("bits_out_neg", '#0000CD', 'out_bps       ');
    $def[$num_graph] .= rrd::line1   ("bits_out_neg", '#32CD32') ;
...
if(($display_errors == 1) && (isset($RRDFILE[4]))){
    ...
    $def[$num_graph] .= rrd::cdef    ("pkt_out_neg", "pkt_out_err,-1,*");
    $def[$num_graph] .= rrd::cdef    ("pkt_in_neg", "pkt_in_err,-1,*");

    $def[$num_graph] .= rrd::area    ("pkt_in_neg",      '#FFD700', 'in_err              ');
    $def[$num_graph] .= rrd::line1   ("pkt_in_neg",      '#000000' ) ;
    $def[$num_graph] .= rrd::gprint  ("pkt_in_err", array("LAST","MAX","AVERAGE"), "%5.1lf%S");
    $def[$num_graph] .= rrd::area    ("pkt_out_neg",     '#FF8C00', 'out_err             ', 'STACK');
    $def[$num_graph] .= rrd::line1   ("pkt_out_neg",     '#000000' ) ;
    $def[$num_graph] .= rrd::gprint  ("pkt_out_err", array("LAST","MAX","AVERAGE"), "%5.1lf%S");
    $def[$num_graph] .= rrd::area    ("pkt_in_discard",  '#7B68EE', 'in_discard          ' );
    $def[$num_graph] .= rrd::line1   ("pkt_in_discard",  '#000000' ) ;
    ...
    $def[$num_graph] .= rrd::line1   ("pkt_out_discard", '#000000' ) ;
