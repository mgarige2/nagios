...
#datacenter = socket.gethostname().split(.)[1]
...
spool_directory = '/nagiostmpfs/perfdata/graphios_lax.dump'

# graphios log info
log_file = '/usr/local/nagios/var/graphios_lax.log'
...
def build_carbon_metric(graphite_prefix, host_name, graphite_postfix):
    ...
    carbon_string = ""
    if graphite_prefix != "":
        graphite_prefix = datacenter_replace(graphite_prefix)
...
def datacenter_replace(string):
    datacenter = str(os.uname()[1].split('.')[:-2][-1:]).strip('[\']')
    return string.replace('.dc.','.'+datacenter+'.')
